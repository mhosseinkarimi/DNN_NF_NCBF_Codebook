...
class FCDNN:
    def __init__(self, num_layers, units, input_shape, output_dim, dropout, loss, model_save_dir=None):
        self.model = self.create_model(num_layers, units, input_shape, output_dim, dropout)
        self.structure = [input_shape] + units + [output_dim]
        self.loss = loss
    ...
    def create_model(self, num_layers, units, input_shape, output_dim, dropout):
        input_ = Input(shape=input_shape)
        x = input_
        for i in range(num_layers):
            x = Dense(units=units[i], activation="relu")(x)
